...
namespace WeifenLuo.WinFormsUI.Docking
{
    ...
    partial class DockPanel
    {
        ...
        private static readonly object ContentClosingEvent = new object();
        [LocalizedCategory("Category_PropertyChanged")]
        [LocalizedDescription("DockPanel_ContentClosing_Description")]
        public event FormClosingEventHandler ContentClosing
        {
            add { Events.AddHandler(ContentClosingEvent, value); }
            remove { Events.RemoveHandler(ContentClosingEvent, value); }
        }
        internal virtual void OnContentClosing(object sender, FormClosingEventArgs e)
        {
            FormClosingEventHandler handler = (FormClosingEventHandler)Events[ContentClosingEvent];
            if (handler != null)
                handler(sender, e);
        }

        private static readonly object ContentClosedEvent = new object();
        [LocalizedCategory("Category_PropertyChanged")]
        [LocalizedDescription("DockPanel_ContentClosedEvent_Description")]
        public event FormClosedEventHandler ContentClosed
        {
            add { Events.AddHandler(ContentClosedEvent, value); }
            remove { Events.RemoveHandler(ContentClosedEvent, value); }
        }
        internal virtual void OnContentClosed(object sender, FormClosedEventArgs e)
        {
            FormClosedEventHandler handler = (FormClosedEventHandler)Events[ContentClosedEvent];
            if (handler != null)
                handler(sender, e);
        }
    }
