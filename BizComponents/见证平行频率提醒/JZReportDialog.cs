...
{
    public partial class JZReportDialog : Form
    {
        public JZReportDialog()
        {
            InitializeComponent();
        }

        private void JZReportDialog_Load(object sender, EventArgs e)
        {
            ComboBox_Segments.Items.Clear();
            ComboBox_Segments.Items.Add("全部标段");
            ComboBox_Segments.SelectedIndex = 0;

            DateTimePicker_Start.Text = DateTime.Now.AddDays(-30).ToString("yyyy年M月dd日");
            ...
            FpSpread_Info.ColumnHeader.Cells[0, 5].Text = "标准见证频率(%)";
            ...
            FpSpread_Info.ColumnHeader.Cells[0, 7].Text = "见证次数";
            FpSpread_Info.ColumnHeader.Cells[0, 7].Tag = "pxCount";
            FpSpread_Info.Columns[7].Width = 100;

            FpSpread_Info.ColumnHeader.Cells[0, 8].Text = "见证频率(%)";
        ...
        private void Button_Export_Click(object sender, EventArgs e)
        {
            ...
            if (DialogResult.OK == folderBrowserDialog1.ShowDialog())
            {
                ...
                String fileName = Path.Combine(fullPath, string.Format("见证频率({0}-{1}-{2}).xls", DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day));
                FpSpread.SaveExcel(fileName, IncludeHeaders.ColumnHeadersCustomOnly);
                MessageBox.Show(string.Format("见证频率“{0}”导出完成", fileName));
            }
        }
    }
}
