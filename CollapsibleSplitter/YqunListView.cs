...
namespace Yqun.Controls
{
	...
	///	</summary>
	public class YqunListView	: System.Windows.Forms.ListView
	{
        ...
        private const int GroupHeight = 58;
        public void EnsureVisible1(int GroupId)
        {
            ...
            if (Groups[GroupId].Items.Count > 0)
            {
                Rectangle Rect = GetSubItemBounds(Groups[GroupId].Items[0], 0);

                GroupRect.X = Rect.X;
                GroupRect.Y = Rect.Y - GroupHeight;
                GroupRect.Width = Rect.Width;
                GroupRect.Height = GroupHeight;

                ScrollToRectangle(GroupRect);
            }
        }
        ...
        public void ScrollToRectangle(Rectangle bounds)
        {
            int dx = 0;
            int scrollToLeft = bounds.X + bounds.Width + MARGIN;
            if (scrollToLeft > this.Bounds.Width)
            {
                dx = scrollToLeft - this.Bounds.Width;
            }
            else
            {
            ...
            {
                int scrollToBottom = bounds.Y + bounds.Height;
                if (scrollToBottom > this.Bounds.Height)
                {
                    dy = scrollToBottom - GroupHeight - 5;
                }
            }
        ...
        public void ScrollToItem(Rectangle bounds)
        {
            int dx = 0;
            int scrollToLeft = bounds.X + bounds.Width + MARGIN;
            if (scrollToLeft > this.Bounds.Width)
            ...
            else
            {
                ...
                if (scrollToBottom > this.Bounds.Height)
                {
                    dy = scrollToBottom - this.Bounds.Height;
                }
            }
        ...
        private void Scroll(int horizontalDx, int verticalDy)
        {
            SendMessage(this.Handle, LVM_SCROLL, (IntPtr)horizontalDx,
            (IntPtr)verticalDy);
        }
	}
}
